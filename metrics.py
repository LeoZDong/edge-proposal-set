...
def false_neg(edges_pred, gt_edges):
    ...
    for edge in edges_pred:
        if tuple(edge) in gt_edges_cp:
            gt_edges_cp.remove(tuple(edge))
...
def metric_wrap(userEmbeds, movieEmbeds, k, gt_edges, metric):
    ...
    if metric == "recall":
        return recall(zip(rows, cols), gt_edges)
    elif metric == "precision":
        return precision(zip(rows, cols), gt_edges)
    elif metric == "precision_recall":
        return precision_recall(zip(rows, cols), gt_edges)
