...
def get_data(csv_file, feat_dim):
    with open(csv_file, newline='') as csvfile:
        ...
        return data, len(userIds), len(movieIds), edge_set, mp_mask, sup_mask, val_mask, test_mask
...
def get_data_cached(csv_file='ratings.csv', feat_dim=128, write_new_file=False):
    ...
    if not write_new_file and os.path.exists(file_name):
        ...
        return [dct[k] for k in ret_keys]
    
    else:
        print(f"File does not exist: {file_name}, creating")
        data, num_users, num_movies, edge_set, mp_mask, \
            sup_mask, val_mask, test_mask = get_data(csv_file, feat_dim)
        torch.save(dict(data=data, mp_mask=mp_mask, sup_mask=sup_mask, 
                        val_mask=val_mask, test_mask=test_mask,
                       num_users=num_users, num_movies=num_movies, edge_set=edge_set), file_name) 
        return data, num_users, num_movies, edge_set, mp_mask, sup_mask, val_mask, test_mask
