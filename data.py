    ...
    with open(csv_file, newline='') as csvfile:
        ...
        for row in csvreader:
            ...
            if rating > THRESHOLD:
                ...
                num_user = max(num_user, edge[0])
                num_item = max(num_item, edge[1])
        num_user += 1  # Because it is zero-indexed
        num_item += 1
...
def get_data_cached(csv_file='ratings.csv', feat_dim=128, write_new_file=False):
    ...
    if not write_new_file and os.path.exists(file_name):
        ...
        torch.save(dict(data=data), file_name)
...
def main():
    ...
    # args.batch_size = 32
    # args.shuffle = True
    # get_data(csv_file='ratings.csv', feat_dim=128)
    data = get_data_cached()
    dataset = get_dataset(data, 'train', 1)
